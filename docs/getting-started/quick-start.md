...
Для обычного использования (Inference/Evaluation) достаточно клонировать основной репозиторий. Стабильные версии пакетов будут установлены автоматически.
...
:::info
Если вы планируете заниматься разработкой (модификацией пакетов в `packages/`), обратитесь к [Contributor Guide](../contributor-guide/development-workflow.md).
:::
...
- **[Концепция архитектуры](../architecture/concept.md)** — зачем микропакеты и три режима работы

### Настройка под себя
- **[Настройка окружения](installation.md)** — Python, CUDA backends, детальная настройка
- **[Управление зависимостями](../contributor-guide/project-structure.md)** — `uv add`, extras, обновления

### Ежедневная работа
- **[Запуск скриптов](../guides/running-benchmarks.md)** — `uv run` vs активация venv
- **[Добавление пакетов](../contributor-guide/development-workflow.md)** — git submodules, workspace

### Продакшен
- **[Docker и деплой](../guides/docker-deployment.md)** — сборка образов, CI/CD
